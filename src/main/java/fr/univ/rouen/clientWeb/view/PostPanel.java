...
public class PostPanel extends JPanel {
	...
	private void placeComponents() {
		...
		JPanel p = new JPanel(new GridBagLayout());
		{
			...
			JScrollPane jS = new JScrollPane(description);
			addToPanel(p, jS, 1, 3, GridBagConstraints.HORIZONTAL, 1., 1.);

			addToPanel(p, new JLabel("Commentaire : "), 0, 4, GridBagConstraints.NONE, 0.1, 1.);
			jS = new JScrollPane(commentaire);
			addToPanel(p, jS, 1, 4, GridBagConstraints.HORIZONTAL, 1., 1.);
		}
		...
		p = new JPanel(new GridBagLayout());
		{
			...
			JScrollPane jS = new JScrollPane(listeMembres);
			jS.setPreferredSize(new Dimension(300, 300));
			addToPanel(p, jS, 1, 0, GridBagConstraints.BOTH, 1., 1.);
		}
		...
		p = new JPanel(new GridBagLayout());
		{
			JPanel q = new JPanel(new GridBagLayout());
			{
				...
				JScrollPane jS = new JScrollPane(descriptionFonctionnalite);
				addToPanel(q, jS, 1, 0, GridBagConstraints.HORIZONTAL, 0.1, 1.);
				...
				JPanel r = new JPanel(new GridBagLayout());
				{
					...
					jS = new JScrollPane(descriptionExigence);
					addToPanel(r, jS, 1, 1, GridBagConstraints.HORIZONTAL, 0.1, 1.);
			...
			JScrollPane jS = new JScrollPane(listeFonctionnalites);
			jS.setPreferredSize(new Dimension(300, 300));
			addToPanel(p, jS, 1, 0, GridBagConstraints.BOTH, 1., 1.);
		}
		...
		ajoutExigence.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (controlField(identifiantExigence, "Veuillez entrer un identifiant d'exigence.")
					...
					try {
						exigences.add(new Exigence(identifiantExigence.getText(), descriptionExigence.getText(),
								Integer.parseInt(prioriteExigence.getText())));
		...
		ajoutFonctionnalite.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (controlField(descriptionFonctionnalite, "Veuillez entrer une description de fonctionnalite.")
					...
					try {
						fonctionnalites.add(new Fonctionnalite(Integer.parseInt(prioriteFonctionnalite.getText()),
								descriptionFonctionnalite.getText(), exigences));
					} catch (NumberFormatException n) {
						JOptionPane.showMessageDialog(PostPanel.this,
								"La priorite de la fonctionnalite n'est pas un entier.");
						return;
					}
					exigences = new ArrayList<Exigence>();
					afficherFonctionnalites();
		...
		depot.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (controlField(titre, "Veuillez entrer un titre.")
					...
					try {
						// A gerer : l'ID
						StbModelVO stb = new StbModelVO(ClientWeb.getInstance().getMaxIndex() + 1, titre.getText(),
								Double.parseDouble(version.getText()), date.getText(), description.getText(), client,
								team, fonctionnalites, commentaire.getText());
